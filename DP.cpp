...
DP::DP(int M, int N, int* c, int* a, int* b)	:M(M),N(N)
{
    feasible = true;
...
void DP::solve()
{
	...
	{	
		...
		sum = 0;
		//1.1
		for (int i = 0; i < M + N; i++) // A * D, tj. vektor * matice
		{
			for (int j = 0; j < M + N; j++)
			{
				sum += A[j] * D[j][i];
...
int DP::operator()(unsigned int i, unsigned int j)
{
    return X[i][j];
}
